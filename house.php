...
$sql;
if($flag){
    $sql="SELECT post_date,rant,datiles,post_id,house.user_id AS u_id,user_name 
    FROM house 
    INNER JOIN users 
            ...
            <label for="inputState">Category</label>
                <select name="category" id="inputState" class="form-control" required>
                    <?php if($flag) { ?>
                        ...
                        <option value="<?php echo $cat3 ?>"><?php echo $cat3 ?></option>
                    <?php }  else { ?>
...
<?php
			...
			<div class="border border-5">
				...
				?>
                    <img src="<?php echo $pic['picture_location']?>" alt=""  width="200" height="180" style="margin-left:30px">
                <?php $I+=1; if($I==3)break; }?>
			<br>
			<a href="ShowDetails.php?id=<?php echo $post['post_id']?>&p-type=house&u_id=<?php echo $post['u_id']?>"class="btn btn-info" style="margin-left:450px;margin-top:5px">View more details</a>
			</div>
			<?php } ?>
</div>
</div>
